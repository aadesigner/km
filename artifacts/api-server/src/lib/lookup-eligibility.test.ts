...
describe("lookup eligibility", () => {
  it("blocks plausible years before MIN_VEHICLE_LOOKUP_YEAR", () => {
    expect(MIN_VEHICLE_LOOKUP_YEAR).toBe(2009);
    expect(isVehicleTooOldForLookup(2008)).toBe(true);
    expect(isVehicleTooOldForLookup(1999)).toBe(true);
    expect(isVehicleEligibleForHistoryLookup(2008)).toBe(false);
  });

  it("allows 2009 and newer", () => {
