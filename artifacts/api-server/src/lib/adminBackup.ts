...
export const BACKUP_FORMAT = "kmcheck-admin-backup";
...
export async function streamBackupExport(res: Response): Promise<{ counts: Record<string, number> }> {
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="kmcheck-backup-${stamp}.json.gz"`,
  );
...
async function gunzipFileToJsonPath(srcPath: string): Promise<string> {
  const dest = path.join(
    tmpdir(),
    `kmcheck-backup-inflated-${Date.now()}-${Math.random().toString(36).slice(2)}.json`,
  );
