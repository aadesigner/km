...
export function HeroVinForm({
  ...
  return (
    <form onSubmit={onSubmit} className={cn("max-w-lg sm:max-w-xl w-full mx-auto space-y-3 text-left", className)}>
      ...
      <div className={cn("sm:hidden", disabled && "opacity-60 pointer-events-none")}>
        <div
          className={cn(
            "hero-vin-card relative overflow-hidden rounded-2xl",
            "bg-background/95 dark:bg-[#0a120e]/95",
            "border border-border/45 dark:border-white/[0.08]",
        ...
        >
          ...
          >
            <div className="hero-vin-input-wrap vin-scanner relative overflow-hidden flex items-center gap-2 p-2 pb-1.5">
              <div className="relative flex min-w-0 flex-1 items-center">
                <Search className="absolute left-3 h-5 w-5 text-primary/60 dark:text-primary/50 shrink-0 z-10 pointer-events-none" />
                <Input
                  className={cn(
                    "h-11 w-full min-w-0 pl-11 pr-2 text-[16px]",
      ...
      <div className={cn("hidden sm:block", disabled && "opacity-60 pointer-events-none")}>
        ...
        {error && <p className="text-sm text-destructive px-1 mt-3">{error}</p>}
