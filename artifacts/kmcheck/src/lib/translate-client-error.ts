...
export function translateClientError(t: TFn, code?: string, error?: string): string {
  ...
  if (code === "PASSWORD_TOO_SHORT" || matchError(error, [/at least 6 characters/i, /at least 8 characters/i])) {
    return t("reset_password_too_short");
  }
  if (code === "PASSWORD_NEEDS_LETTER" || matchError(error, [/include a letter/i])) {
